...
async def set_anti_func(chat_id, status, mode):
    anti_f = nexaub_antif.find_one({"_id": chat_id})
    if anti_f:
        nexaub_antif.update_one(
            {"_id": chat_id}, {"$set": {"status": status, "mode": mode}}
        )
    else:
        nexaub_antif.insert_one({"_id": chat_id, "status": status, "mode": mode})
...
async def del_anti_func(chat_id):
    anti_f = nexaub_antif.find_one({"_id": chat_id})
    if anti_f:
        nexaub_antif.delete_one({"_id": chat_id})
        return True
    else:
        return False
...
async def url_on(chat_id, status):
    anti_f = lockurl.find_one({"_id": chat_id}, {"$set": {"status": status}})
    if anti_f:
        lockurl.update_one({"_id": chat_id})
    else:
        lockurl.insert_one({"_id": chat_id, "status": status})


async def is_urlon(chat_id):
    anti_f = lockurl.find_one({"_id": chat_id})
    if not anti_f:
        return None
...
async def url_off(chat_id):
    anti_f = lockurl.find_one({"_id": chat_id})
    if anti_f:
        lockurl.delete_one({"_id": chat_id})
        return True
    else:
        return False
