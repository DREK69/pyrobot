...
async def rban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_bot_admin(chat, bot.id) or not bot_member.can_restrict_members:
        await message.reply_text(
...
async def runban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Remote unban a user from a chat"""
    bot, args = context.bot, context.args
    message = update.effective_message
...
async def rkick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Remote kick/punch a user from a chat"""
    bot, args = context.bot, context.args
    message = update.effective_message
    ...
    try:
        # For kick, we ban then immediately unban
        await chat.ban_member(user_id)
        await chat.unban_member(user_id)
...
async def runmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Remote unmute a user in a chat"""
    bot, args = context.bot, context.args
    message = update.effective_message
    ...
    try:
        ...
        if excp.message == "Reply message not found":
            ...
            LOGGER.warning(update)
            LOGGER.exception(
                "ERROR unmuting user %s in chat %s (%s) due to %s",
                user_id,
                chat.title,
                chat.id,
                excp.message,
...
# Handler registration - PTB v22 style
def register_handlers():
    """Register all remote moderation handlers"""
    
    # Create handlers with proper filters
