...
from MerissaRobot import DEV_USERS, DRAGONS, application
...
async def connect_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in DRAGONS):
                ...
                if connection_status:
                    conn_chat = await application.bot.get_chat(
            ...
            if conn:
                connectedchat = await application.bot.get_chat(conn)
        ...
        if (isadmin) or (isallow and ismember) or (user.id in DRAGONS):
            ...
            if connection_status:
                chat_name = (await application.bot.get_chat(chat.id)).title
                await send_message(
                    update.effective_message,
