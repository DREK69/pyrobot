...
from MerissaRobot.Database.sql.welcome_sql import get_current_welcome_settings
...
async def multibutton(event):
    ...
    if time:
        if not time == 0:
            asyncio.create_task(
                kick_restricted_after_delay(WELCOME_DELAY_KICK_SEC, event, user_id)
            )
...
async def math(event):
    ...
    buttons = Button.url(
        ...
        "t.me/MerissaRobot?start=math_{}".format(event.chat_id),
    )
...
async def h(event):
    ...
    keyboard = [
        [
            Button.inline(f"{a}", data="sikle_{}".format(chat)),
            Button.inline(f"{b}", data="babe_{}".format(chat)),
        ],
        [
            Button.inline(f"{c}", data="nide_{}".format(chat)),
            Button.inline(f"{d}", data="nipa_{}".format(chat)),
        ],
    ]
    shuffle(keyboard)
    await asyncio.sleep(0.5)
...
async def bak(event):
    ...
    await event.edit(
        "Successfully Verified✅, now you can message in the chat!", buttons=None
    )
    try:
...
async def bak(event):
    ...
    if maths == 0:
        ...
        return await event.edit(
            "Your chances are exchausted, verification failed❌", buttons=None
        )
...
async def text(event):
    ...
    if cws:
        ...
        if last:
            ...
            fullname = first
        userid = a_user.id
        current_saved_welcome_message = cws.custom_welcome
        text = current_saved_welcome_message.format(
            mention=mention,
            title=title,
            first=first,
    ...
    buttons = Button.url(
        "Click here to prove you are human",
        "t.me/MerissaRobot?start=captcha_{}".format(event.chat_id),
    )
    await event.reply(text, buttons=buttons)
    WELCOME_DELAY_KICK_SEC = time
    if time:
...
async def h(event):
    ...
    c = gen_captcha(8)
    d = gen_captcha(8)
    image_captcha.generate_image(a)
    image_file = "./" + "captcha.png"
    image_captcha.write(a, image_file)
    keyboard = [
        [
            Button.inline(f"{a}", data="pip_{}".format(chat)),
            Button.inline(f"{b}", data="exec_{}".format(chat)),
        ],
        [
            Button.inline(f"{c}", data="sli_{}".format(chat)),
            Button.inline(f"{d}", data="paku_{}".format(chat)),
        ],
    ]
    shuffle(keyboard)
    await asyncio.sleep(0.5)
    await tbot.send_message(
        event.chat_id,
        "Please choose the text from image",
        file="./captcha.png",
        buttons=keyboard,
    )
...
async def cbot(event):
    ...
    await event.answer("❌ Wrong Try Again!")
    keyboard = [
        Button.inline(f"{emoji.BRAIN}", data=f"pep-{user_id}"),
        Button.inline(f"{emoji.CHECK_MARK_BUTTON}", data=f"pro-{user_id}"),
        Button.inline(f"{emoji.CROSS_MARK}", data=f"fk-{user_id}"),
        Button.inline(f"{emoji.ROBOT}", data=f"yu-{user_id}"),
    ]
    shuffle(keyboard)
...
async def t(event):
    ...
    try:
        ...
        await event.reply(
            f"Turned on captcha kick time to **{time}s**/nNow new users who don't complete captcha by **{time}s** gets automatically kicked!"
        )
...
async def button(event):
    ...
    buttons = Button.inline(
        "Click Here to prove you're Human", data=f"check-bot-{user_id}"
    )
    cws = get_current_welcome_settings(event.chat_id)
    if cws:
        a_user = await event.get_user()
...
async def t(event):
    ...
    if (
        ...
        and not arg == "text"
        and not arg == "math"
        and not arg == "multibutton"
    ):
        return await event.reply(
            f"'{arg}' is not a recognised CAPTCHA mode! Try one of: button/multibutton/math/text"
        )
    type = arg
    if type:
        for c in chats:
            if event.chat_id == c["id"]:
                ...
                return
        captcha.insert_one({"id": event.chat_id, "type": type, "time": time})
    ...
    if arg:
        if arg == "on" or arg == "enable" or arg == "yes":
            if mode:
                if mode == "on":
                    return await event.reply(
                        "Captcha is already enabled for this chat."
                    )
                elif mode == "off":
                    to_check = get_chat(id=event.chat_id)
                    captcha.update_one(
            ...
            if mode:
                if mode == "off":
                    return await event.reply(
                        "captcha is not enabled here in the first place!"
                    )
                elif mode == "on":
                    to_check = get_chat(id=event.chat_id)
                    captcha.update_one(
