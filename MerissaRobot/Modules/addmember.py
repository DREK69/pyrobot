   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Sorry Sir, You are Banned. Contact My [Support Group](https://t.me/PrincexSupport).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
         ...
         await app.send_message(chat_id=message.chat.id, text="**Something Went Wrong. Contact My [Support Group](https://t.me/PrincexSupport).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
...
async def start(lel, message):
   ...
   but = InlineKeyboardMarkup([[InlineKeyboardButton("❓ 𝘾𝙤𝙢𝙢𝙖𝙣𝙙 𝙃𝙚𝙡𝙥 ❗️", callback_data="help")], [InlineKeyboardButton("𝙏𝙚𝙧𝙢𝙨 𝘼𝙣𝙙 𝘾𝙤𝙣𝙙𝙞𝙩𝙞𝙤𝙣𝙨❗️", callback_data="terms")],[InlineKeyboardButton(text="📇 𝙐𝙥𝙙𝙖𝙩𝙚𝙨", url="http://t.me/PrincexBots"), InlineKeyboardButton(text="🫂 𝙎𝙪𝙥𝙥𝙤𝙧𝙩", url="https://t.me/PrincexSupport")],[InlineKeyboardButton("𝘼𝙙𝙢𝙞𝙣 𝙋𝙖𝙣𝙚𝙡 👨‍💻", callback_data="Admin")]])
   await message.reply_photo(photo="https://telegra.ph/file/1698f5639ff1c7451c9d3.jpg", caption=f"**Hᴇʟʟᴏ** `{message.from_user.first_name}` !\n\n× **I'ᴍ Pʀɪɴᴄᴇ Sᴄʀᴀᴘᴇʀ Bᴏᴛ. Yᴏᴜ Cᴀɴ Sᴄʀᴀᴘ Mᴇᴍʙᴇʀꜱ Fʀᴏᴍ Aɴᴏᴛʜᴇʀ Gʀᴏᴜᴘꜱ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ Uꜱɪɴɢ Tʜɪꜱ Bᴏᴛ.**\n\n**Cʀᴇᴀᴛᴇᴅ Bʏ** @PrincexBots", reply_markup=but)
...
async def phone(lel, message):
 try:
   ...
   with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
      ...
      number = await app.ask(chat_id=message.chat.id, text="**Enter number of accounts to Login (in intiger) Like 1, 2, 3.**")
      n = int(number.text)
      a+=n
      if n<1 :
      ...
      for i in range (1,n+1):
         ...
         if "+" in phone:
            ...
            await app.send_message(message.chat.id, """**Invalid Mobile Number Format Try again**""") 
...
async def to(lel, message):
 try:
   number = await app.ask(chat_id=message.chat.id, text="**Now Enter Group Username Or Link From You Want Scrap Members.**")
   From = number.text
   number = await app.ask(chat_id=message.chat.id, text="**Now Enter Group Username Or Link You Want To Add Members.**")
   To = number.text
   number = await app.ask(chat_id=message.chat.id, text="**Now Enter Number like 1, 2, 3.**")
   a = int(number.text)
   di=a
   try:
      with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
...
async def subscribers_count(lel, message):
   if message.from_user.id in OWNER:
      but = InlineKeyboardMarkup([[InlineKeyboardButton("Users", callback_data="Users"), InlineKeyboardButton("Broadcast", callback_data="Broadcast")], [InlineKeyboardButton("Add User", callback_data="New"), InlineKeyboardButton("Check Users", callback_data="Check")]])
      await app.send_message(chat_id=message.chat.id,text=f"**Hi** `{message.from_user.first_name}` **!\n\nWelcome to Admin Panel**", reply_markup=but)
   else:
      await app.send_message(chat_id=message.chat.id,text="**You are not Admin of This Bot**")
...
async def button(app, update):
   ...
   if "tutorial" in k:
      ...
      if update.message.chat.id in OWNER:
         ...
         await app.send_message(chat_id=update.message.chat.id,text=f"**Welcome to Admin Panel**", reply_markup=but)
      else:
    ...
    try:
      ...
      a=0
      b=0
      number = await app.ask(chat_id=update.message.chat.id, text="**Now Send me Message For Broadcast**")
      phone = number.text
      for row in query:
         chat_id = int(row[0])
         try:
