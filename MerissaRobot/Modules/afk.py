...
from MerissaRobot import REDIS
from MerissaRobot.Database.redis.afk_redis import (
    afk_reason,
    end_afk,
    is_user_afk,
    start_afk,
)
from MerissaRobot.Handler.decorators import merissacmd, merissamsg
...
def afk(update, _):
    ...
    try:
        message.reply_video(
            AFK_VID,
            caption=f"Baii Baii <code>{fname}</code> 👋",
...
@merissamsg((Filters.all & Filters.chat_type.groups), friendly="afk", group=AFK_GROUP)
def no_longer_afk(update, _):
    ...
    if res:
        ...
        try:
            message.reply_video(
                AWAKE_VID,
                caption=f"<code>{firstname}</code> Just Leave Phone for Only <code>{end_afk_time}</code>",
                parse_mode=ParseMode.HTML,
            )
        except BadRequest:
            return
