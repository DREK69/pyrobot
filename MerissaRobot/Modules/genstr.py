...
async def _callbacks(pbot, callback_query):
    ...
    if query.startswith("generate"):
        ...
        try:
            if query == "pyrogram":
                await generate_session(pbot, callback_query.message)
            else:
                await generate_session(lbot, callback_query.message, telethon=True)
        ...
        try:
            if query == "cbpyrogram":
                await withoutapigenerate(pbot, callback_query.message)
            else:
                await withoutapigenerate(pbot, callback_query.message, telethon=True)
        except Exception as e:
            await callback_query.message.reply(ERROR_MESSAGE.format(str(e)))
                ...
                [
                    [
                        InlineKeyboardButton(
                            "Pyrogram Music-Bot", callback_data="cbpyrogram"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            "Telethon User-Bot", callback_data="cbtelethon"
                        )
                    ],
                ]
            ),
...
@pbot.on_message(filters.private & ~filters.forwarded & filters.command("genstr"))
...
Press Button Below to Start Generating Session!""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔗 Generate String With Your API", callback_data="generate"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "🔗 Generate String With Bot's API", callback_data="cbgenerate"
                    )
                ],
            ]
        ),
...
async def generate_session(pbot, msg, telethon=False):
    await msg.reply(
...
async def withoutapigenerate(bot, msg, telethon=False):
    await msg.reply(
        "Starting {} Session Generation...".format(
            "Telethon" if telethon else "Pyrogram"
        )
