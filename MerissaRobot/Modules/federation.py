...
from MerissaRobot import telethn as xbot
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id:
        ...
        previous_message.sender.first_name
    else:
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if (
        int(user_id) in fed_admins
        or int(user_id) == OWNER_ID
        or int(user_id) == 1100231654
    ):
        return True
    else:
        return False
...
async def new(event):
    ...
    if fedowner:
        ...
        return await event.reply(
            f"You already have a federation called `{text}` ; you can't create another. If you would like to rename it, use /renamefed."
        )
    if not name:
        return await event.reply(
    ...
    fed_name = name
    sql.new_fed(event.sender_id, fed_name, fed_id)
    return await event.reply(
...
async def cgname(event):
    ...
    return await event.reply(
        f"Tada! I've renamed your federation from '{name}' to '{newname}'. [FedID: `{fed_id}`]."
    )
...
async def cf(event):
    ...
    name = info["fname"]
    await event.reply(
        f"Chat {event.chat.title} is part of the following federation: {name} [ID: `{fed_id}`]"
    )
...
async def lf(event):
    ...
    return await event.reply(
        f'Chat {event.chat.title} has left the " {name} " federation.'
    )
...
async def smex_fed(event):
    ...
    if res:
        return await event.edit(
            f"User [{fname}](tg://user?id={user}) is now an admin of {name} [{fed_id}]"
        )
...
async def smex(event):
    ...
    if event.sender_id == int(owner):
        ...
        fname = rt.user.first_name
        await event.edit(
            f"Fedadmin promotion cancelled by [{fname}](tg://user?id={owner})"
        )
        return
    if event.sender_id == int(user):
        ...
        fname = rt.user.first_name
        await event.edit(
            f"Fedadmin promotion has been refused by [{fname}](tg://user?id={user})."
        )
        return
...
async def _(event):
    ...
    await xbot.send_message(event.chat_id, sax)
    getfednotif = sql.user_feds_report(info["owner"])
    if getfednotif:
        if int(info["owner"]) != int(chat):
            await xbot.send_message(int(info["owner"]), sax)
    get_fedlog = sql.get_fed_log(fed_id)
    if get_fedlog:
        if int(get_fedlog) != int(chat):
            await xbot.send_message(int(get_fedlog), sax)
        ...
        for fedsid in subscriber:
            ...
            for fedschat in all_fedschat:
                try:
                    await xbot(
                        EditBannedRequest(
                            int(fedschat), int(fban_user_id), BANNED_RIGHTS
                        )
                    )
                except Exception:
                    continue
...
async def unfban(event):
    ...
    if not event.reply_to_msg_id:
        ...
        iid = reply_message.sender_id
        fname = reply_message.sender.first_name
        reply_message.sender.last_name
    ...
    if temp:
        ...
        if reason:
            sxa += f"**Reason:** {reason}"
        await xbot.send_message(event.chat_id, sxa)
        getfednotif = sql.user_feds_report(info["owner"])
        if getfednotif:
            if int(info["owner"]) != int(chat):
                await xbot.send_message(int(info["owner"]), sxa)
        get_fedlog = sql.get_fed_log(fed_id)
        if get_fedlog:
            if int(get_fedlog) != int(chat):
                await xbot.send_message(int(get_fedlog), sxa)
...
async def sub(event):
    ...
    sql.subs_fed(args, fed_id)
    sql.add_sub(fed_id, args)
    await event.reply(
        f"Federation {name} has now subscribed to {sname}. All fedbans in {sname} will now take effect in both feds."
    )
...
async def unsub(event):
    ...
    if not getfed:
        return await event.reply("This FedID does not refer to an existing federation.")
    sname = getfed["fname"]
    sql.rem_sub(fed_id, args)
    sql.unsubs_fed(args, fed_id)
    await event.reply(
...
async def fstat(event):
    ...
    if args:
        if len(args) > 12:
            ...
            if fbanreason == "":
                text = (
                    f"{fname} is currently banned in {name}.\n\n**Date of Ban:** {rs}"
                )
            if not fbanreason == "":
